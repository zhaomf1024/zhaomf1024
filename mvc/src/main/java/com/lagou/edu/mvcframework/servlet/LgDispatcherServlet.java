        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(method.isAnnotationPresent(Security.class)) {
                    ...
                    securityValue = security.value(); // 等同于/demo
                }

                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url), securityValue);
...
//        method.invoke() //
        ...
        if(handler.getSecurityValue().size() >0){
            String username = req.getParameter("username");
            if(username == null || username.length() ==0){
                resp.getWriter().write("没有访问权限");
                return;
            }
            if(!handler.getSecurityValue().contains(username)){
                resp.getWriter().write("没有访问权限");
                return;
            }
        }
